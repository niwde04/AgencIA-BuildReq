...
const FLOW_DESCRIPTIONS: Record<QueueFlowType, string> = {
  despacho_bodega:
    "Crea una salida de inventario en borrador para la requisición seleccionada.",
  compra_directa:
    "Genera una solicitud de compra directa para luego convertirla en OC.",
...
const FLOW_ROUTE_HINTS: Record<QueueFlowType, string> = {
  despacho_bodega:
    "Se atiende aquí, genera una salida SB en borrador y continúa en Salidas de Inventario.",
  compra_directa:
    "Se atiende aquí, genera una SC y luego continúa en Solicitudes de Compra.",
  traslado_proyecto:
    "Se atiende aquí y luego continúa en Solicitudes de Traslado.",
  solicitud_compra:
    "Se atiende aquí y luego continúa en Solicitudes de Compra.",
};
...
const getDispatchStockOptionsForRow = (row: PendingQueueRow) =>
  (
    (row.item?.dispatchStockOptions ??
      row.item?.projectStockWarehouses ??
      []) as any[]
  ).filter(
    option =>
...
const getDispatchWarehouseOptionsForRow = (row: PendingQueueRow) => {
  ...
  return ((row.project?.warehouses ?? []) as any[]).filter(
    warehouse =>
      getDispatchWarehouseOptionId(warehouse) > 0 &&
...
const formatSupplierReferenceLabel = (
  reference:
    | {
        supplierCode?: string | null;
        supplierName?: string | null;
  ...
  return (
    reference?.supplierName || reference?.supplierCode || "Proveedor pendiente"
  );
};
...
export default function Flujos() {
  ...
  const [processingFlowType, setProcessingFlowType] =
    useState<QueueFlowType | null>(null);
  const [returningQueuedFlowType, setReturningQueuedFlowType] =
    useState<QueueFlowType | null>(null);
  const [returningDispatchItemId, setReturningDispatchItemId] = useState<
    number | null
  >(null);
  const [returningTransferItemId, setReturningTransferItemId] = useState<
    number | null
  >(null);
  const [dispatchNotesByFlowType, setDispatchNotesByFlowType] = useState<
    Partial<Record<QueueFlowType, string>>
  >({});
  const [dispatchQuantityByItemId, setDispatchQuantityByItemId] = useState<
    Record<number, string>
  >({});
  const [dispatchWarehouseByItemId, setDispatchWarehouseByItemId] = useState<
    Record<number, string>
  >({});
  const [dispatchSourceProjectByItemId, setDispatchSourceProjectByItemId] =
    useState<Record<number, string>>({});
  const [dispatchCheckedByItemId, setDispatchCheckedByItemId] = useState<
    Record<number, boolean>
  >({});
  const [directPurchaseNotesByFlowType, setDirectPurchaseNotesByFlowType] =
    useState<Partial<Record<QueueFlowType, string>>>({});
  const [directPurchaseCheckedByItemId, setDirectPurchaseCheckedByItemId] =
    useState<Record<number, boolean>>({});
  const [directPurchaseQuantityByItemId, setDirectPurchaseQuantityByItemId] =
    useState<Record<number, string>>({});
  ...
  const [transferNotesByFlowType, setTransferNotesByFlowType] = useState<
    Partial<Record<QueueFlowType, string>>
  >({});
  const [purchaseRequestTypeByFlowType, setPurchaseRequestTypeByFlowType] =
    useState<Partial<Record<QueueFlowType, string>>>({});
  const [purchaseRequestNotesByFlowType, setPurchaseRequestNotesByFlowType] =
    useState<Partial<Record<QueueFlowType, string>>>({});
  const [purchaseRequestCheckedByItemId, setPurchaseRequestCheckedByItemId] =
    useState<Record<number, boolean>>({});
  ...
  const directPurchaseMutation =
    trpc.supplyFlows.createDirectPurchaseBatch.useMutation();
  const projectTransferBatchMutation =
    trpc.supplyFlows.createProjectTransferBatch.useMutation();
  const purchaseRequestMutation =
    trpc.supplyFlows.createPurchaseRequest.useMutation();
  const purchaseRequestBatchMutation =
    trpc.supplyFlows.createPurchaseRequestBatch.useMutation();
  const warehouseExitBatchMutation =
  ...
  const visibleFlowSections = useMemo(
    () =>
      flowFilter === "all"
        ? allowedFlowTypes
        : allowedFlowTypes.filter(type => type === flowFilter),
  ...
  const processWarehouseDispatchFlow = async (flowType: QueueFlowType) => {
    ...
    if (selectedRows.length === 0) {
      toast.error(
        "Seleccione al menos un detalle para crear salida de inventario"
      );
      return;
    }
    for (const row of selectedRows) {
      ...
      const normalizedSourceProjectId =
        sourceProjectId === UNCLASSIFIED_STOCK_SOURCE_PROJECT_ID
          ? null
          : sourceProjectId;
      const selectedWarehouseQuantity = getDispatchWarehouseOptionQuantity(
        selectedSourceProject
      );
      const singleStorageLocation = getSingleDispatchStorageLocation(
        selectedSourceProject
      );
      ...
      if (
        ...
        toast.error(
          `${item.itemName}: seleccione proyecto/origen del inventario`
        );
        return;
      }
    ...
    try {
      ...
      if (createdExitNumbers.length <= 1) {
        const exitLabel = createdExitNumbers[0]
  ...
  const returnDispatchItemToRequisition = async (row: PendingQueueRow) => {
    ...
    try {
      const result = await returnDispatchMutation.mutateAsync({
        id: row.item.id,
      });
      resetProcessedItemDrafts(
        [row.item.id, result.pendingRequestItemId].filter(Boolean)
      );
      await invalidateAll();
      toast.success(
  ...
  const returnTransferItemToRequisition = async (row: PendingQueueRow) => {
    ...
    try {
      const result = await returnTransferMutation.mutateAsync({
        id: row.item.id,
      });
  ...
  const processTransferFlow = async (flowType: QueueFlowType) => {
    ...
    if (selectedRows.length === 0) {
      toast.error(
        "Seleccione al menos un detalle para la solicitud de traslado"
      );
      return;
    }
    ...
    try {
      ...
      processedItemIds.push(
        ...preparedTransferRows.map(({ row }) => row.item.id)
      );
  ...
  const processPurchaseRequestFlow = async (flowType: QueueFlowType) => {
    ...
    if (failedItems.length === 0) {
      toast.success(
        `Solicitud de compra generada para ${processedItemIds.length} ítem(s)`
      );
          ...
          <Select value={flowFilter} onValueChange={setFlowFilter}>
            ...
            <SelectContent>
              ...
              {allowedFlowTypes.includes("despacho_bodega") && (
                <SelectItem value="despacho_bodega">
              ...
              {allowedFlowTypes.includes("traslado_proyecto") && (
                <SelectItem value="traslado_proyecto">
                  Solicitud de traslado
                </SelectItem>
              )}
              {allowedFlowTypes.includes("solicitud_compra") && (
                <SelectItem value="solicitud_compra">
                  Solicitud de compra
                </SelectItem>
              )}
            </SelectContent>
              ...
              <Card key={flowType} className="border-border/80 bg-background">
                ...
                <CardContent className="space-y-4">
                  ...
                  flowType === "traslado_proyecto" ? (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                      Solo seguimiento: este traslado lo procesa Jefe de Bodega
                      ...
                      <tbody>
                        {rows.map(row => {
                          ...
                          const pendingDispatchQuantity =
                            getPendingDispatchQuantity(item);
                          const availableDispatchQuantity =
                            getAvailableDispatchQuantity(item);
                          const hasAvailableStock =
                            availableDispatchQuantity > 0;
                          const projectWarehouses =
                            getDispatchWarehouseOptionsForRow(row);
                          ...
                          const selectedSourceProject =
                            sourceProjectOptions.find(
                              option =>
                                getDispatchSourceProjectOptionId(option) ===
                                selectedSourceProjectId
                            );
                          const selectedSourceStorageLocation =
                            getSingleDispatchStorageLocation(
                              selectedSourceProject
                            );
                          const selectedSourceQuantity =
                            getDispatchWarehouseOptionQuantity(
                              selectedSourceProject
                            ) ?? 0;
                          ...
                          const selectedDispatchQuantity =
                            parseQuantityValue(dispatchInputValue);
                            ...
                            >
                                ...
                                (flowType === "despacho_bodega" ||
                                  ...
                                  <td className="p-2 text-center">
                                    {flowType === "despacho_bodega" ? (
                                      <Checkbox
                                        checked={
                                          dispatchCheckedByItemId[item.id] ===
                                          true
                                        }
                                        onCheckedChange={checked =>
                                          setDispatchCheckedByItemId(
                                            current => ({
                                              ...current,
                                      ...
                                      <Checkbox
                                        checked={
                                          directPurchaseCheckedByItemId[
                                            item.id
                                          ] === true
                                        }
                                        onCheckedChange={checked =>
                                          setDirectPurchaseCheckedByItemId(
                                            current => ({
                                              ...current,
                                              [item.id]: checked === true,
                                            })
                                        ...
                                        disabled={
                                          isProcessing || isReturningQueued
                                        }
                                      />
                                    ) : flowType === "traslado_proyecto" ? (
                                      <Checkbox
                                        checked={
                                          transferCheckedByItemId[item.id] ===
                                          true
                                        }
                                        onCheckedChange={checked => {
                                      ...
                                      <Checkbox
                                        checked={
                                          purchaseRequestCheckedByItemId[
                                            item.id
                                          ] === true
                                        }
                                        onCheckedChange={checked =>
                                          setPurchaseRequestCheckedByItemId(
                                            current => ({
                                              ...current,
                                              [item.id]: checked === true,
                                            })
                              ...
                              <td className="p-2">
                                ...
                                {flowType === "compra_directa" &&
                                  ...
                                  !row.purchaseInsight?.minimumPurchase && (
                                    <p className="text-[11px] text-muted-foreground">
                                      Sin historial de compra registrado para
                                      este SAP.
                                    </p>
                                  )}
                                {flowType === "despacho_bodega" &&
                              ...
                              {canProcessThisFlow &&
                                flowType === "despacho_bodega" && (
                                  <td className="p-2">
                                    <Select
                                      value={
                                        defaultDispatchWarehouseId || undefined
                                      }
                                      onValueChange={value => {
                                        setDispatchWarehouseByItemId(
                                          current => ({
                                            ...current,
                                            [item.id]: value,
                                          })
                                        );
                                        setDispatchSourceProjectByItemId(
                                          current => {
                                            const next = { ...current };
                                            delete next[item.id];
                                            return next;
                                    ...
                                    >
                                      ...
                                      <SelectContent className="min-w-[var(--radix-select-trigger-width)]">
                                        {projectWarehouses.length === 0 ? (
                                          ...
                                          projectWarehouses.map(
                                            (warehouse: any) => {
                                              const warehouseId =
                                                getDispatchWarehouseOptionId(
                                                  warehouse
                                                );
                                              ...
                                              return (
                                                <SelectItem
                                                  ...
                                                  className="text-xs"
                                                >
                                                  {getDispatchWarehouseOptionLabel(
                                                    warehouse
                                                  )}
                                                  {warehouseQuantity !== null
                                          ...
                                          {sourceProjectOptions.map(
                                            (option: any) => {
                                              const sourceProjectId =
                                                getDispatchSourceProjectOptionId(
                                                  option
                                                );
                                              const quantity =
                                                getDispatchWarehouseOptionQuantity(
                              ...
                              {canProcessThisFlow &&
                                flowType === "compra_directa" && (
                                  <td className="p-2">
                                    <Input
                                      value={
                                        directPurchaseQuantityByItemId[
                                          item.id
                                        ] ?? String(item.quantity ?? "0.00")
                                      }
                                      onChange={event =>
                                        setDirectPurchaseQuantityByItemId(
                                          current => ({
                                            ...current,
                                  ...
                                  <td className="p-2 text-right">
                                    {flowType === "despacho_bodega" ? (
                                      <Button
                                        ...
                                        onClick={() =>
                                          void returnDispatchItemToRequisition(
                                            row
                                          )
                                        }
                                      >
                                      ...
                                      <Button
                                        ...
                                        onClick={() =>
                                          void returnTransferItemToRequisition(
                                            row
                                          )
                                        }
                                      >
                  ...
                  {canProcessThisFlow && flowType === "compra_directa" && (
                    <div className="grid gap-3">
                      ...
                      <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2 text-xs text-muted-foreground">
                        Marca solo los detalles que quieres incluir en esta
                        solicitud. También puedes bajar la cantidad para hacer
                        compras parciales; el resto quedará pendiente en este
                        mismo panel.
                      </div>
                    </div>
                  )}
                  ...
                  {canProcessThisFlow && flowType === "traslado_proyecto" && (
                    <div className="space-y-3">
                      <div className="rounded-lg border border-border/70 bg-muted/15 p-3 text-xs text-muted-foreground">
                        Aquí solo se genera la solicitud. La bodega origen se
                        seleccionará en Solicitudes de Traslado antes de
                        convertirla a traslado.
                      </div>
                  ...
                  {canProcessThisFlow && flowType === "solicitud_compra" && (
                    <div className="space-y-3">
                      ...
                      <div className="rounded-lg border border-border/70 bg-muted/15 px-3 py-2 text-xs text-muted-foreground">
                        Si seleccionas más de un ítem del mismo proyecto, se
                        generará una sola solicitud de compra consolidada.
                      </div>
                    </div>
                  )}
                  ...
                  {canProcessThisFlow && (
                    <div className="flex flex-wrap justify-end gap-3 border-t border-border/70 pt-3">
                      ...
                      {flowType === "compra_directa" && (
                        <Button
                          onClick={() =>
                            void processDirectPurchaseFlow(flowType)
                          }
                      ...
                      {flowType === "despacho_bodega" && (
                        <Button
                          onClick={() =>
                            void processWarehouseDispatchFlow(flowType)
                          }
                          disabled={isProcessing}
                        >
                          {isProcessing
                      ...
                      {flowType === "solicitud_compra" && (
                        <Button
                          onClick={() =>
                            void processPurchaseRequestFlow(flowType)
                          }
                          disabled={isProcessing || isReturningQueued}
                        >
                          {isProcessing
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1 space-y-0.5">
                            ...
                            {row.flow.paymentMethod && (
                              <p className="text-xs text-muted-foreground">
                                Método:{" "}
                                {getPaymentMethodLabel(row.flow.paymentMethod)}
                              </p>
                            )}
                            ...
                            {row.flow.purchaseOrderNumber && (
                              <p className="text-xs text-muted-foreground">
                                {row.flow.sapDocumentType === "solicitud_compra"
                                  ? "SC"
                                  : "OC"}
                                : {row.flow.purchaseOrderNumber}
                              </p>
